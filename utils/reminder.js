        ...
        for (const task of allTasks) {
            ...
            if (!task.dueDate) {
                ...
                try {
                    ...
                    shouldInclude = true; // 日付解析できない場合は含める
                }
            } else {
                // 期限ありタスクは一週間以内のもの
...
export async function testDailyReminder(client, interaction) {
    try {
        await interaction.reply({
            content: '毎朝通知のテストを実行します（一週間以内のタスクのみ）',
            ephemeral: true
        });
        ...
        embed.setFooter({ text: 'テスト実行完了 (一週間以内のタスクを表示)' });
        ...
        await interaction.followUp({
            content: `テスト実行エラー: ${error.message}`,
            ephemeral: true
        });
    }
}
